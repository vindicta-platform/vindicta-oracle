...
class DebateEngine:
    ...
    def __init__(self, config: OllamaConfig | None = None, num_rounds: int = 3, context_assembler: "ContextAssembler | None" = None):
        ...
        Args:
            ...
            context_assembler: Optional assembler for RAG rules pre-fetching
        """
    ...
    async def run_debate(self, context: DebateContext) -> DebateTranscript:
        ...
        if self.context_assembler:
            import re
            units = set()
            # simple heuristic: look for "- UnitName" or "UnitName" patterns if listed.
            for text in (context.player1_list, context.player2_list):
                if not text:
                    continue
                for line in text.split("\n"):
                    match = re.search(r"-\s*(?:[0-9]+x\s+)?([A-Za-z0-9\s]+?)(?:\s*\(|\s*\[|$)", line)
                    if match:
            ...
            if units:
                rules_pkg = await self.context_assembler.assemble(list(units))
                context.rules_context = rules_pkg
                transcript.rag_segments_used = [str(s.segment_id) for s in rules_pkg.segments]
                print(f"\n📚 RAG Context Assembled: {len(rules_pkg.segments)} segments via {rules_pkg.retrieval_mode.value}.")
    ...
    async def run_grading_session(self, army_list: "meta_oracle.models.ArmyList") -> DebateTranscript:
        ...
        return await self.run_debate(context)
